...
- 重构知识图谱静态布局，分离关系节点与孤立节点的布局流程，增加碰撞消解和外围环带定位。
- 增强节点和关系原文定位：支持实体、关系、关系说明和出处文本块的分层高亮，并兼容旧版图谱页面的数据回查。
- 新增文档预览、源码和富文本编辑模式，支持草稿保存、文档版本还原、文件列表增量更新，以及增量更新后的自动图谱重绘。
- 图谱历史升级为图谱与文档联合快照，图谱还原后前端会同步刷新文档和引用数据。
- 增加文件切换未保存保护和异步内容请求序号，避免快速切换小文件时编辑区出现空白或旧内容覆盖新内容。
- 重做四种主题的颜色变量和高亮色阶，降低高饱和色和纯黑纯白对比，主题切换时清理旧变量并平滑过渡。
- 重新组织文档工具栏，将查看模式、高亮设置、复制下载和编辑历史操作分组排列，改善窄屏布局。
...
- 文件列表细分: 文件库-文件,文件库也可构建图谱用于将多个文件的图谱进行融合或者对比相似度等功能
- 数据库与数据库也可建立宏观联系
- 完成用户图谱构建习惯(采用机器学习,对比llm生成初始图和用户剪枝后的差别,进行习惯化学习帮助用户删除不必要的节点和关系)
...
完整条款见 [LICENSE](LICENSE)，双许可说明见 [COMMERCIAL-LICENSE.md](COMMERCIAL-LICENSE.md)。
